...
- **合同管理（P3-2）**：合同实体（V15）+ 状态机 `DRAFT → PENDING_APPROVAL → EFFECTIVE → EXECUTING → CLOSED` + `CHANGE_REQUEST`（注册进 StateMachineRegistry）；金额/交期/供应商由定标结果注入（不来自 LLM）；AI 草拟（模式 B：确定性模板 + 条款库软提示，只进草拟文本）+ 条款风险识别（模式 A+C：条款级风险分级 + 原因）+ 后端一致性校验（草拟文本金额/交期必须与注入字段一致，不一致拦截审批）；必填条款（金额/交期）校验；变更单重新审批（allow-once），旧条款快照留痕；合同中心页（列表/详情/审批/变更），任务详情定标后显示「生成合同」入口；评测 `frozen-evaluation-contract.json`（synthetic，条款提取 precision/recall 100% + 字段注入一致性硬校验）。
