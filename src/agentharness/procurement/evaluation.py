...
def _xlsx_bytes(workbook: Workbook) -> bytes:
    ...
    stable = io.BytesIO()
    # Use ZIP_STORED (not DEFLATE) so the generated bytes are independent of the
    # zlib build/version and therefore reproducible across machines and CI.
    with source, zipfile.ZipFile(stable, "w", compression=zipfile.ZIP_STORED) as target:
