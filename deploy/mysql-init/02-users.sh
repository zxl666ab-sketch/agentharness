...
: "${AGENT_DATABASE_USER:?AGENT_DATABASE_USER is required}"
...
case "$PROCUREMENT_DATABASE_USER$AGENT_DATABASE_USER" in
...
valid_secret "$AGENT_DATABASE_PASSWORD" || { echo "AGENT_DATABASE_PASSWORD must be a 32+ character safe secret" >&2; exit 1; }
...
CREATE USER IF NOT EXISTS '$AGENT_DATABASE_USER'@'%' IDENTIFIED BY '$AGENT_DATABASE_PASSWORD';
ALTER USER '$AGENT_DATABASE_USER'@'%' IDENTIFIED BY '$AGENT_DATABASE_PASSWORD';
GRANT ALL PRIVILEGES ON caijiatai_runtime.* TO '$AGENT_DATABASE_USER'@'%';
FLUSH PRIVILEGES;
SQL
